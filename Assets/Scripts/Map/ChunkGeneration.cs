...
public class ChunkGeneration : MonoBehaviour
{
    ...
    void Generation()
    {
        ...
        for (int y = 0; y < 16; y++)
        {
            for (int x = 0; x < 16; x++)
            {
                if (noiseMap[x+112, y+112] < shallowWaterLevel){
                    float noiseValue = 0.4f;
                    noiseMap[x + 112, y + 112] = noiseValue;
                }
        ...
        for (int x = 0; x < width; x++)
        {
            for (int y = 0; y < height; y++)
            {
                ...
                if (cell.isDeepWater)
                {
                    waterMap.SetTile(new Vector3Int(x, y, -5), deepWater);
                }
            ...
            if (buildable == surface)
            {
                for (int x = 0; x < largeur; x++)
                {
                    for (int y = 0; y < hauteur; y++)
                    {
                        ...
                        buildingMap.SetTile(new Vector3Int(randx + x, randy + y, -5), material);
                    }
                }
            }
        }
